...
R_HOME_DIR=
if test "${R_HOME_DIR}" = "/usr/local/R-3.1.2/lib64/R"; then
   case "linux-gnu" in
   linux*)
     run_arch=`uname -m`
     case "$run_arch" in 
     ...
     if [ -x "/usr/local/R-3.1.2/${libnn}/R/bin/exec/R" ]; then
        R_HOME_DIR="/usr/local/R-3.1.2/${libnn}/R"
     elif [ -x "/usr/local/R-3.1.2/${libnn_fallback}/R/bin/exec/R" ]; then
        R_HOME_DIR="/usr/local/R-3.1.2/${libnn_fallback}/R"
